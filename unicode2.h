...
class Unicode
{
	...
	*Using the above fact the characters are seperated and stored as data type Data, defined above.
	...
	private:
        ...
        Data *single_utf8;//Pointer to processed data and its info
	public:
        void scan_RawInput();
        void input_CommandLine();
        int length_Unicode();
        std::string operator+(int);
        int size_UnicodeChar(int);
...
int Unicode::size_UnicodeChar(int i)
{
...
int Unicode::length_Unicode()
{
    ///Return length of unicode string
	return unicode_length;
}
void Unicode::input_CommandLine()
{
	///Will be replaced..Replaced!! See operator=.
    std::cout<<"Enter the unicode string"<<std::endl;
    getline(std::cin,raw_data);
    scan_RawInput();
}
void Unicode::scan_RawInput()
{
	/**
	  ...
	  */
	 Data *info=new Data[raw_data.length()+1];
	/*
	...
	info[j].position=i;
	unicode_length=j;
	single_utf8=new Data[unicode_length+1];
	/*
